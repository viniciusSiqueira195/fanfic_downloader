"""Adaptador da pesquisa e dos livros recentes do Visionvox."""
...
class Visionvox:
    ...
    def explorar_pagina(self, formato="epub", pagina=0, idioma="pt", topico=""):
        ...
        if topico != "visionvox:recentes":
            return PaginaLivros([], False,
                                " O Visionvox oferece descoberta pelos livros recentes.")
        deslocamento = pagina * 50
        parametros = {
            "estante": "recente", "formato": formato,
            "pagina": "Nao" if pagina == 0 else "sim",
            "num_page": deslocamento,
        }
        if pagina > 0 and formato in self._paginas_recentes:
            parametros["total_pagina"] = self._paginas_recentes[formato]
        resposta = self.http.get(
            urljoin(BASE_URL, "biblioteca/recente.php"),
            params=parametros,
            headers=HEADERS, timeout=(5, 20),
        )
        resposta.raise_for_status()
        html = resposta.content.decode("utf-8")
        livros = [livro for livro in extrair_livros(html) if livro.formato == formato]
        soup = BeautifulSoup(html, "html.parser")
        total_paginas = self._paginas_recentes.get(formato, 0)
        for link in soup.find_all("a", href=True):
            consulta = parse_qs(urlsplit(link["href"]).query)
            total = consulta.get("total_pagina", [""])[0]
            if total.isdecimal():
                total_paginas = max(total_paginas, int(total))
        if total_paginas:
            self._paginas_recentes[formato] = total_paginas
        return PaginaLivros(livros, pagina + 1 < total_paginas)
